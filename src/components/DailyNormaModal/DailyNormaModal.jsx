...
import { DailyNormaWrapper, WaterRateWrapper, DailyNormaHeader, WaterRateParagraph, DailyNormaButton, ModalWrapper,Modalsvg, AmountWrapper, ModalLabelWater, InputWrapperWater, ButtonWrapper, InputWrapper, ModalHeader, GenderWrapper, ModalParagraphForGirl,
    ModalParagraphForMan, ModalSpan, ModalInfo, FormulaWrapper, ModalForm, ModalCalc, ModalLabel, ModalRadioInput, RadioLabel, ModalInput, Amount,ModalButton, WaterIntakeSpan } from './DailyNormaModalStyled'
    ...
    const Modal = ({ isOpen, close, children }) => {
      ...
      const styles = {
        overlay: {
          ...
          right: 0,
          bottom: 0,
          backgroundColor: 'rgba(5, 7, 8, 0.7)',
          display: 'flex',
          alignItems: 'center',
...
export const DailyNorma = () => {
    ...
    return (
      ...
      ><DailyNormaModal   onSubmit={handleWaterRateUpdate}/>
      </Modal>
...
const DailyNormaModal = ({onClose}) => {
  ...
  const [waterIntakeInput, setWaterIntakeInput] = useState('0');
    ...
    <ModalRadioInput id="forWoman" type="radio" value="For girl" checked={gender === 'For girl'} onChange={() => setGender('For girl')} />
          ...
          <ModalInput type="text" value={waterIntakeInput}  onChange={handleInputChange} min={1} max={15000} placeholder='0'/>
          </InputWrapperWater>
